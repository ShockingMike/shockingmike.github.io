...
async function fontsReady() {
  ...
  const sample = 'KERN SOCIETY Chớm Studio Kōzō Hadal Perihelion Sắp phát hành COMING SOON SMR 001 33⅓';
...
async function boot() {
  ...
  const records = RECORDS.map((r) => (r.art && !images[r.id] ? { ...r, sealed: true } : r));
  renderTicks();
  renderPages();
  // the back of each sleeve is printed once, in English, like the credits on a real record
  const credits = Object.fromEntries(RECORDS.map((r) => {
    const c = COPY.en.records[r.id];
