// Chớm, mùa Hạ (from people/dong): loose white sketch loops round the picker, as core's sketchAround draws them round a hero
...
export async function buildSketch(actors, paintGeo, core, per = {}, eyeAt = null, Y = async () => {}) {
  ...
  const plans = actors.map((a) => {
    ...
    const idx = [];
    for (let i = 0; i < pos.count; i++) if (Math.round(aChar.getX(i)) === a.index && (i % 11) === 0) idx.push(i);
  ...
  function linesNow(plan) {
    ...
    const pts = [];
    let zc = 0;
    // (the eye in the space the person lives in: the boat's, for the drawing now on the bones)
    const eye = eyeAt ? eyeAt(a.cur) : REF.eye;
    for (const i of idx) {
      skinVert(i, _w).sub(eye);
    ...
    return loops.map((L) => {
      ...
      for (let k = 0; k <= L.span; k++) {
        ...
        line.push(eye.clone().addScaledVector(REF.fwd, zc).addScaledVector(REF.right, X * zc).addScaledVector(REF.up, Y * zc));
      }
